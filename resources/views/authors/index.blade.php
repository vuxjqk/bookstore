...
@section('title', __('Author management'))
    ...
    <div class="py-6">
        ...
        <nav class="bg-gray-50 px-6 py-3 text-gray-700">
            <ol class="list-reset flex text-sm">
                ...
                <li class="text-gray-500">{{ __('Author management') }}</li>
            </ol>
        </nav>
        ...
        <main class="p-6">
            ...
            <div class="flex items-center justify-between mb-6">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900">{{ __('Author management') }}</h1>
                    <p class="text-gray-600 mt-1">{{ __('List of all authors in the system') }}</p>
                </div>
                <div>
                    <a href="{{ route('authors.create') }}"
                        class="px-4 py-2 text-white bg-blue-600 rounded-lg hover:bg-blue-700 transition-colors duration-200">
                        <i class="fas fa-plus mr-2"></i>{{ __('Add new author') }}
                    </a>
                </div>
            </div>
                ...
                <form class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <x-text-input id="search" class="block mt-1 w-full" type="search" name="search"
                            :value="request('search')" placeholder="{{ __('Author name, slug...') }}" />
                    </div>
            ...
            <div class="bg-white rounded-lg shadow overflow-hidden">
                <div class="px-6 py-4 border-b border-gray-200">
                    <h3 class="text-lg font-semibold text-gray-900">
                        <i class="fas fa-list mr-2 text-green-500"></i>{{ __('Author list') }}
                    </h3>
                </div>
                            ...
                            @foreach ($authors as $author)
                                <tr class="hover:bg-gray-50">
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                        {{ $author->name }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $author->slug }}
                                    </td>
                                    <td class="px-6 py-4 text-sm text-gray-500">
                                        {{ $author->description ?? __('No description') }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                        <div class="flex space-x-2">
                                            ...
                                            <button
                                                onclick="openDeleteModal('{{ route('authors.destroy', $author->id) }}')"
