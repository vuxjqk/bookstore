    ...
    <div class="py-6">
        ...
        <nav class="bg-gray-50 px-6 py-3 text-gray-700">
            <ol class="list-reset flex text-sm">
                ...
                <li class="text-gray-500">{{ __('Supplier management') }}</li>
            </ol>
        </nav>
        ...
        <main class="p-6">
            ...
            <div class="flex items-center justify-between mb-6">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900">{{ __('Supplier management') }}</h1>
                    <p class="text-gray-600 mt-1">{{ __('List of all suppliers in the system') }}</p>
                </div>
                <div>
                ...
                <form class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <x-text-input id="search" class="block mt-1 w-full" type="search" name="search"
                            :value="request('search')" placeholder="{{ __('Supplier name, slug...') }}" />
                    </div>
            ...
            <div class="bg-white rounded-lg shadow overflow-hidden">
                <div class="px-6 py-4 border-b border-gray-200">
                    <h3 class="text-lg font-semibold text-gray-900">
                        <i class="fas fa-list mr-2 text-green-500"></i>{{ __('Supplier list') }}
                    </h3>
                </div>
                            ...
                            @foreach ($suppliers as $supplier)
                                <tr class="hover:bg-gray-50">
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                        {{ $supplier->name }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $supplier->slug }}
                                    </td>
                                    <td class="px-6 py-4 text-sm text-gray-500">
                                        {{ $supplier->description ?? __('No description') }}</td>
